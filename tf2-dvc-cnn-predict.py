...
# Initialization:
if len(sys.argv)<2:
    print('ERROR: model file path missing')
    sys.exit()
...
if len(sys.argv) > 2:
    ...
    image_path = f'dogs-vs-cats/train-2000/validation/cats/cat.12000.jpg'
image = load_image( os.path.join(os.environ.get('DATADIR'), image_path))

# Inference:
prediction = model.predict(tf.expand_dims(image, 0))
print('Predicted label for "' + image_path[image_path.rindex("/")+1:] + '" is: ' +
      classes[int(np.round(prediction).item())])
print('All done')
